...
```{r}
#| label: tbl-1
#| tbl-cap: "Some summary statistics split by category"
cont_df %>%
group_by(a) %>%
...
```{r}
#| label: tbl-2
#| tbl-cap: "Some grouped summary statistis."
...
```{r}
#| label: tbl-3
#| tbl-cap: "Emotion Regulation Questionnaire items with means and stnadard deviations"
text_df <- data.frame(
...
```{r}
#| label: tbl-4
#| tbl-cap: "Some stats from models with interactions"
x <- rnorm(1e3)
x_2 <- rnorm(1e3, 3, .7)
